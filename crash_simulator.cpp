...
// 模拟非法内存访问
void simulateAccessViolation() {
    int* ptr = (int*)0x12345678;  // 设置非法内存地址
    log("[AccessViolation] 正在访问非法内存地址...");
    std::cout << *ptr << std::endl;  // 访问非法内存地址，会导致崩溃
}
...
void simulateDeadlock() {
    ...
    t1.join();
    t2.join();  // 这两个线程将因相互等待而无法结束，形成死锁
}

// 模拟崩溃（通过注入）
void simulateCrash() {
    log("[Crash] 模拟崩溃...");
    simulateNullPointerCrash();  // 可以选择其他崩溃类型：simulateMemoryLeak() 或 simulateDeadlock()
}
